...
for subj in `cat $code_DIR/subjids/subjids.txt`; do
	for SES in ${data_DIR}/sub-${subj}/ses-nsd*/; do
		ses=`basename "$SES"`		
		for i in ${data_DIR}/sub-${subj}/${ses}/func/sub-${subj}_${ses}_task-nsdcore_run-*_bold.nii.gz; do
			if [ -f "${i}" ]; then
				if [ "${counter}" -lt 36 ] ; then
				counter=$((counter+1))
				(echo 00$counter ${i}) >> ${data_DIR}/sub-${subj}/sub-${subj}_TASK_fmrinii.txt
				else
				echo ""
				fi
			else
