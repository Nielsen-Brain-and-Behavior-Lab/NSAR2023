...
out_dir=${HOME_D}/HCPD_analysis/parc_output_fs6_HCPD_REST
preproc_output=${HOME_D}/HCPD_analysis/CBIG2016_preproc_FS6_REST
...
#1. create list of sessions available for each subject - MULTIPLE SESSIONS OF REST
for sub in `cat ${code_dir}/subjids/MultipleSessions_230412_ids.txt`; do
...
#1. AGAIN create list of sessions available for each subject - SPLIT SESSIONS (SINGLE RUN SPLIT)

for sub in `cat ${code_dir}/subjids/1sess_230412_ids.txt`; do
	for sess in 1 2 3 4 5 6 7 8 9; do
		for split in 1 2; do
		FILE=$out_dir/generate_profiles_and_ini_params/profiles/sub${sub}/sess${sess}/\
